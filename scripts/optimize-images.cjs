...
// Try to require sharp from node_modules
let sharp;
try {
  ...
  console.log('⚠️  Sharp not found, trying alternative paths...');
  ...
  for (const p of possiblePaths) {
    try {
      sharp = require(p);
      console.log('✅ Sharp loaded from:', p, '\n');
      break;
    } catch (e2) {
      continue;
    }
...
async function optimizeImages() {
  if (!sharp) {
    ...
    // Fallback: Copy images with .webp extension for Next.js to process
    console.log('📝 Creating placeholder .webp files for Next.js optimization...\n');
    for (const img of IMAGES_TO_OPTIMIZE) {
      ...
      if (fs.existsSync(inputPath) && !fs.existsSync(outputPath)) {
        ...
        console.log(`✓ Created ${path.basename(img.output)} (will be optimized by Next.js)`);
      }
    }
    console.log('\n✨ Done! Next.js will optimize these images at runtime.');
    return;
  }
